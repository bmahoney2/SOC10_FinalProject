...
# make the interface with questions 
ui <- fluidPage(
...
# Only use the features from the lasso (others are zero)
    ...
    tryCatch({
      ...
      output$riskOutput <- renderUI({
        tagList(
          ...
          p("**DISCLAIMER:** this tool is NOT to be used as a proxy for making medical diagnoses. All results should be considered
          with the discresion of medical professionals.")
        )
      })
    })
  })
